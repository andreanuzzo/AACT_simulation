...
- $T_{inf}$ = Duration of the _infectious_period_ (~2.9 days, from literature)
...
Ct_infection_curves <- res_ct  %>% 
  ...
  xlab('Time since beginning of recording (days)') + 
  ylab('Infected')+
  scale_y_continuous(labels = unit_format(unit = "M", scale = 1e-6), limits = c(0,3.5e7))+
...
Ct_exp_curves <- res_ct  %>% 
  ...
  scale_discrete_manual(values = colorRampPalette(colors = c("#F2F5A9", '#e09200'))(10), 
                        ...
                        name = 'Intervention strength')+
  xlab('Time since beginning of recording (days)') + 
  scale_y_continuous(labels = unit_format(unit = "M", scale =  1e-6), limits = c(0,7e7))+
  ylab('Exposed')+
  geom_hline(yintercept = 1e6, linetype = 'dotdash') +
  xlim(c(0,200))
...
```{r eval=FALSE, fig.height=21, fig.width=7, include=FALSE}
...
v <- res_cta  %>% 
  ...
  scale_y_continuous(labels = unit_format(unit = "M", scale = 1e-6), limits = c(0,3.5e7))+
  geom_hline(yintercept = 1e6, linetype = 'dotdash') +
  facet_grid(rows = 'pop_responsibility') 

direct.label(v, method = 'top.bumptwice')
